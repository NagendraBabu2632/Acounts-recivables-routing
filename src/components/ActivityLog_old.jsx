...
// // ─── PTP Tracker ──────────────────────────────────────────────────────────────
// export function PTPTracker({ refreshKey }) {
//   const [ptps, setPtps]   = useState([])
//   const [loading, setLoading] = useState(false)

//   const load = () => {
//     setLoading(true)
//     api.ptps().then(r=>setPtps(r.ptps||[])).catch(console.error).finally(()=>setLoading(false))
//   }
...
//       <div style={{padding:'12px 16px',borderBottom:'1px solid var(--border)',flexShrink:0,
//         display:'flex',alignItems:'center',justifyContent:'space-between'}}>
//         <span style={{fontFamily:'var(--font-head)',fontSize:13}}>PTP Tracker</span>
//         {broken.length>0 && (
//           <span className="badge badge-critical">{broken.length} broken</span>
//         )}
//       </div>
//       {broken.length>0 && (
//         <div style={{margin:'8px 12px',padding:'8px 12px',background:'rgba(239,68,68,0.08)',
//           borderRadius:'var(--radius-sm)',border:'1px solid rgba(239,68,68,0.25)'}}>
//           <div style={{fontSize:11,color:'var(--risk-critical)',fontWeight:600,marginBottom:2}}>
//             ⚠ {broken.length} broken promise(s) — escalation required
//           </div>
//           {broken.map(p=>(
//             <div key={p.ptp_id} style={{fontSize:11,color:'var(--text-secondary)'}}>
...
//           <div style={{padding:16,textAlign:'center',color:'var(--text-muted)',fontSize:12}}>No PTP records</div>
//         )}
//         {ptps.map((p,i)=>{
//           const isB  = p.status==='broken'
//           const isO  = p.status==='open'
//           const bCol = isB?'var(--risk-critical)':isO?'var(--accent-blue)':'var(--risk-low)'
//           return (
//             <div key={i} style={{margin:'0 12px 8px',padding:'10px 12px',
//               background:'var(--bg-surface)',borderRadius:'var(--radius-md)',
//               border:`1px solid ${isB?'rgba(239,68,68,0.3)':'var(--border)'}`}}>
//               <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
//                 <span style={{fontSize:12,fontWeight:500,color:'var(--text-primary)'}}>{p.customer_name}</span>
//                 <span style={{fontSize:10,padding:'2px 7px',borderRadius:10,
//                   background:`${bCol}18`,color:bCol,border:`1px solid ${bCol}33`}}>
//                   {p.status.toUpperCase()}
//                 </span>
//               </div>
//               <div style={{display:'flex',justifyContent:'space-between',marginTop:6}}>
//                 <span style={{fontSize:13,fontFamily:'var(--font-mono)',fontWeight:600,color:bCol}}>
//                   ₹{(p.amount/100000).toFixed(2)}L
//                 </span>
//                 <span style={{fontSize:11,color:'var(--text-muted)'}}>by {p.promise_date}</span>
//               </div>
//               <div style={{fontSize:10,color:'var(--text-muted)',marginTop:4}}>
//                 {p.contact} · {p.mode_of_payment||'NEFT'} · {p.created_date}
//               </div>
//             </div>
//           )
//         })}
